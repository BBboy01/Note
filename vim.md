...
- `*` 或 `#` 包含单词边界快速全局搜索当前光标所在的单词
- `g*` 或 `g#` 不包含单词边界全局搜索当前光标所在的单词
...
| 命令                                | 功能                                          |
| ----------------------------------- | --------------------------------------------- |
| :g/{pattern}/{excmd}                | 对满足 pattern 的行文本进行 excmd 操作        |
...
|         $         |                                      跳到当前行尾                                      |
|         ^         |                              跳到当前行首第一个非空的位置                              |
|        \_         | 跳到当前行首第一个非空的位置, 但可以结合数字前缀，用于跳转到下面几行的第一个非空白字符 |
|        g\_        |                            跳转到当前行的最后一个非空白字符                            |
|        gg         |                                       跳到第一行                                       |
|         G         |                                      跳到最后一行                                      |
|         %         |                                 在闭合的 \(\) 之间移动                                 |
|        \{         |                                     移动至上一段落                                     |
...
| :/\{range\}g/\{place\}/s/\{place\}/\{result\}/g | 将 range 行中包含 place 行的 target 替换为 result |
|                      <C-v>                      |             选择完成后按 I/A 进入编辑             |
