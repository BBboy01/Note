...
`yarn add -D prettire eslint-plugin-prettier eslint-config-prettier`
...
> 编辑其中的`pre-commit`来设置 git 钩子命令

它将根据`package.json`依赖项中的代码质量工具来安装和配置husky和`lint-staged`，因此请确保在此之前安装（`npm install --save-dev`）并配置所有代码质量工具，如Prettier和ESlint
...
然后执行 `npx conventional-changelog -p angular -i CHANGELOG.md -s` 生成 CHANGELOG.md
